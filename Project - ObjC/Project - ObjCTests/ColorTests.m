...
- (void)testColorWithCArray {
    ...
    // With
    CGFloat expectedColors[3][4]
        = {{0.3, 0.3, 0.3, 1.0}, {0.4, 0.0, 1.0, 0.0}, {0.2, 0.3, 0.2, 0.4}};

    // When
    NSArray *resultColors = @[
        [Color colorWithCArray:expectedColors[0]],
        [Color colorWithCArray:expectedColors[1]],
        [Color colorWithCArray:expectedColors[2]]
    ];

    // Then
    for (int i = 0; i < testColors.count; i++) {
        XCTAssertEqualObjects(testColors[i], resultColors[i]);
    }
}
